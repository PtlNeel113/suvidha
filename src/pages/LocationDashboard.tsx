...
import {
  ...
  Thermometer, Wind, CloudRain, LocateFixed, Check,
} from "lucide-react";
...
const availableAreas = [
  ...
  { area: "Thaltej", city: "Ahmedabad", ward: "Ward 15" },
  { area: "Borivali", city: "Mumbai", ward: "Ward 7" },
  { area: "Andheri", city: "Mumbai", ward: "Ward 10" },
...
export default function LocationDashboard() {
  ...
  const { t } = useLanguage();
  const [location, setLocation] = useState({ area: "", city: "", ward: "" });
  ...
  const handleDetectLocation = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          // Simulate reverse geocoding - use coordinates to pick nearest area
          const idx = Math.floor(Math.random() * 5); // Simulated
          setLocation(availableAreas[idx]);
          setDetectingLocation(false);
          setShowLocationPicker(false);
        },
        () => {
          // Fallback to default
          setLocation(availableAreas[0]);
          setDetectingLocation(false);
          setShowLocationPicker(false);
        },
        { timeout: 5000 }
  ...
  const queueData = { counter: { wait: "25 min", people: 12 }, kiosk: { wait: "2 min", people: 1 } };

  // Location Picker Modal
  if (showLocationPicker) {
    return (
      <PageTransition>
        <KioskLayout title={`📍 ${t("choose_location")}`}>
          <div className="max-w-xl mx-auto px-4 py-12">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
              ...
              <h2 className="font-heading text-2xl font-bold mb-2">{t("choose_location")}</h2>
              <p className="text-muted-foreground">{t("detecting_location")}</p>
            </motion.div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full p-5 mb-4 rounded-2xl border-2 border-success/30 hover:border-success bg-gradient-to-r from-success/5 to-success/10 transition-all flex items-center gap-4"
            >
              <div className="w-14 h-14 rounded-full bg-success flex items-center justify-center text-success-foreground">
                {detectingLocation ? (
              ...
              <div className="text-left">
                <p className="font-heading font-bold text-lg">{t("use_live_location")}</p>
                <p className="text-sm text-muted-foreground">
                  {detectingLocation ? t("detecting_location") : "Auto-detect your GPS location"}
                </p>
              </div>
            </motion.button>
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center"><span className="bg-background px-3 text-sm text-muted-foreground">{t("select_area")}</span></div>
            </div>
              ...
              {availableAreas.map((area, idx) => (
                <motion.button
                  ...
                  className={`w-full p-4 rounded-xl border-2 transition-all flex items-center justify-between ${
                    selectedAreaIdx === idx ? "border-secondary bg-secondary/5" : "border-border hover:border-secondary/50"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">{area.ward}</p>
                    </div>
                  </div>
                  {selectedAreaIdx === idx && <Check className="w-5 h-5 text-success" />}
                </motion.button>
              ))}
            </div>
          </div>
  ...
  return (
    <PageTransition>
      <KioskLayout title={`📍 ${t("location_dashboard")}`}>
        <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-heading text-2xl font-bold">📍 {location.area}, {location.city}</h2>
                <p className="text-muted-foreground text-sm flex items-center gap-2">
                  ...
                  <button onClick={() => setShowLocationPicker(true)} className="text-secondary hover:underline text-xs ml-2">Change</button>
                </p>
              </div>
            </div>
          ...
          {/* Emergency Bar */}
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="bg-destructive/10 border-2 border-destructive/30 rounded-2xl p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="font-heading font-bold text-destructive">{t("emergency").toUpperCase()}</span>
                {emergencyNumbers.map((em) => (
                  <motion.a key={em.number} href={`tel:${em.number}`} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              ...
              <motion.button
                onMouseDown={handleSOSStart} onMouseUp={handleSOSRelease} onTouchStart={handleSOSStart} onTouchEnd={handleSOSRelease}
                whileTap={{ scale: 0.95 }}
                className={`relative w-16 h-16 rounded-full font-heading font-bold text-sm ${
                  sosActive ? "bg-destructive text-destructive-foreground" : "bg-destructive/20 text-destructive border-2 border-destructive"
                }`}
              >
                {sosActive ? "SENT!" : "SOS"}
                {sosTimer > 0 && sosTimer < 3 && (
                  <motion.div className="absolute inset-0 rounded-full border-4 border-destructive" initial={{ scale: 1 }} animate={{ scale: 1.3, opacity: 0 }} transition={{ duration: 1, repeat: Infinity }} />
                )}
              </motion.button>
            </div>
            <p className="text-xs text-destructive/70 mt-2">Hold SOS for 3 seconds to alert police + family</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <Card className="p-4">
              ...
              <div className="space-y-2">
                {alerts.map((alert) => (
                  <motion.div key={alert.id} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}
                    className={`flex items-center justify-between p-3 rounded-xl text-sm ${
                      alert.type === "warning" ? "bg-primary/10 border border-primary/20" :
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <Card className="p-4">
                ...
                <div className="space-y-3">
                  {nearbyServices.map((service, idx) => (
                    <motion.div key={idx} whileHover={{ x: 4 }} className="flex items-center justify-between p-3 rounded-xl bg-muted/50 hover:bg-muted cursor-pointer transition-colors">
                      <div className="flex items-center gap-3">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }}>
              <Card className="p-4">
                <h3 className="font-heading font-semibold mb-4">{t("quick_access")}</h3>
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { icon: <Bus className="w-6 h-6" />, label: t("bus_metro"), path: "/bus-pass", color: "bg-secondary" },
                    { icon: <MessageSquare className="w-6 h-6" />, label: t("ward_chat"), path: "/ward-chat", color: "bg-success" },
                    { icon: <Heart className="w-6 h-6" />, label: t("ayushman"), path: "/health", color: "bg-destructive" },
                    { icon: <Sun className="w-6 h-6" />, label: t("schemes"), path: "/schemes", color: "bg-primary" },
                    { icon: <Users className="w-6 h-6" />, label: t("complaints"), path: "/complaints", color: "bg-accent" },
                    { icon: <Zap className="w-6 h-6" />, label: t("electricity"), path: "/electricity", color: "bg-secondary" },
                  ].map((action, idx) => (
                    <motion.button key={idx} onClick={() => navigate(action.path)} whileHover={{ scale: 1.05, y: -3 }} whileTap={{ scale: 0.95 }}
                      ...
                      <div className={`w-12 h-12 rounded-full ${action.color} text-white flex items-center justify-center`}>{action.icon}</div>
          ...
          {/* Bottom Bar */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex justify-center gap-4">
            <Button size="lg" onClick={() => navigate("/")} className="bg-secondary hover:bg-secondary/90 min-h-[56px] px-8">
              🏠 {t("govt_services")}
            </Button>
            <Button size="lg" variant="outline" onClick={() => navigate("/schemes")} className="min-h-[56px] px-8">
              🏛️ {t("check_eligibility")}
            </Button>
            <Button size="lg" variant="outline" onClick={() => navigate("/complaints")} className="min-h-[56px] px-8">
              📝 {t("file_complaint")}
            </Button>
          </motion.div>
        </div>
